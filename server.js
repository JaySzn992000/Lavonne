...
app.post(
  ...
  (req, res) => {
    const { category, name, price, sizes, stock, description, review } = req.body;

    const imagePath = req.files.image ? `/Images/${req.files.image[0].filename}` : null;
    const imagePathOne = req.files.imageone ? `/Images/${req.files.imageone[0].filename}` : null;
    const imagePathTwo = req.files.imagetwo ? `/Images/${req.files.imagetwo[0].filename}` : null;
    const imagePathThree = req.files.imagethree ? `/Images/${req.files.imagethree[0].filename}` : null;
